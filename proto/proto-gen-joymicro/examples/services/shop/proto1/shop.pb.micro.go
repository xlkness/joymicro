...
func NewShopService(etcdAddrs []string, timeout time.Duration, isPermanent bool) ShopService {
	c := client.New("Shop", etcdAddrs, timeout, isPermanent)
	return &shopService{
		c:    c,
		name: "Shop",
	}
...
func RegisterShopHandler(s *server.ServicesManager, hdlr ShopHandler, peerInfo *server.Peer2Peer) error {
	err := s.RegisterOneService("Shop", hdlr, peerInfo)
	return err
}
...
func (h *shopHandler) Buy(ctx context.Context, in *Request, out *Response) error {
